...
int ledState = 0;
int counter = 0;
int halfBufferReady = 0;  // Flag for half buffer ready
int fullBufferReady = 0;  // Flag for full buffer ready
int readCounter = 0;  // Counter for read cycles
int writeCounter = 0;  // Counter for write cycles
__attribute__((section(".sram1"), aligned(32))) int16_t audioBuffer[AUDIO_BUFFER_SIZE];
...
__attribute__((section(".sram1"), aligned(32))) uint16_t audioInputBuffer[AUDIO_ADC_CHANNELS];  // DMA buffer for direct ADC data
...
ps_t pulsar; // Pulsar state structure
/* USER CODE END PV */
...
void writeSineCycle(int16_t *buffer, uint16_t size);
void configurePulsarState(void);
void writePulsarAudio(int16_t *buffer, uint16_t size);
/* USER CODE END PFP */
...
int main(void)
{
  ...
  status = HAL_ADC_Start_DMA(&hadc3, (uint32_t *) audioInputBuffer, AUDIO_ADC_CHANNELS);
  HAL_TIM_Base_Start(&htim1);  

  // DMA for I2S
  writeSineCycle(audioBuffer, AUDIO_BUFFER_SIZE);  // Fill the audio buffer with a sine wave
  status = HAL_I2S_Transmit_DMA(&hi2s1, (uint16_t *) audioBuffer, AUDIO_BUFFER_SIZE);
  ...
  while (1)
  {
    ...
    // if (halfBufferReady) {
    //   configurePulsarState();
    //   writePulsarAudio(audioBuffer, AUDIO_BUFFER_SIZE / 2); // Process the first half of the buffer
    //   halfBufferReady = 0;
    //   writeCounter++;
    // }
    // if (fullBufferReady) {
    //   configurePulsarState();
    //   writePulsarAudio(audioBuffer + (AUDIO_BUFFER_SIZE / 2), AUDIO_BUFFER_SIZE / 2); // Process the first half of the buffer
    //   fullBufferReady = 0;
    // }

    // turn on the LED if the write counter falls behind the read counter
    if (readCounter - writeCounter > 1) {
      HAL_GPIO_WritePin(LED_GPIO_PORT, PITCH_LED_PIN, GPIO_PIN_RESET);
    } else {
      HAL_GPIO_WritePin(LED_GPIO_PORT, PITCH_LED_PIN, GPIO_PIN_SET);
    }

    HAL_GPIO_WritePin(LED_GPIO_PORT, CADENCE_LED_PIN, ledState ? GPIO_PIN_SET : GPIO_PIN_RESET); // Toggle the cadence LED
    HAL_Delay(500); // Delay to control the LED blink rate
...
void configurePulsarState(void) {
  static float potValues[5] = { 0.5, 0.0, 0.5, 0.0, 0.0 }; // Placeholder for potentiometer values
...
void writePulsarAudio(int16_t *buffer, uint16_t size) {
  const float scaleFactor = (-1.0 / 2.0) * 32768;
  static float pitch = 0.5 * 500.0 + 35.0;
  for (int i = 0; i < size; i += 2) {
    buffer[i] = scaleFactor * pulsar_process(&pulsar, pitch, 0);
    buffer[i + 1] = buffer[i];
  }
}
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc) {
  if (hadc->Instance == ADC1) {
    ...
    // Do something with the ADC2 data
    adcBuffer[0] = adcBuffer[0];
  }
  else if (hadc->Instance == ADC3) {
    // Do something with the ADC3 data
    audioInputBuffer[0] = audioInputBuffer[0];
  }
}
